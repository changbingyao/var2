<template>
    <div>
        <!--列表功能-->
        <br>
        <UserRoleQuery :selectedIds="selectedIds"
                   @add="handleAdd"
                   @loadData="loadData"/>
        <br>
        <!--列表主体-->
        <div>
            <el-row>
                <el-table
                   ...
                   <el-table-column
                           :label="$t('userRole.userRoleId')"
                           prop="userRoleId" show-overflow-tooltip>
                   </el-table-column>
                   <el-table-column
                           :label="$t('userRole.userCode')"
                           prop="userCode" show-overflow-tooltip>
                   </el-table-column>
                   <el-table-column
                           :label="$t('userRole.roleCode')"
                           prop="roleCode" show-overflow-tooltip>
                   </el-table-column>
                   <el-table-column
                           :label="$t('userRole.crtUserCode')"
                           prop="crtUserCode" show-overflow-tooltip>
                   </el-table-column>
                    <el-table-column :label="$t('action')" min-width="90px">
                        <template slot-scope="scope">
                            ...
                            <el-button type="text" v-permission="'del','删除'" @click="handleDelete(scope.row.userRoleId)">{{$t('delete')}}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            </el-row>
        ...
        <div>
            <el-row>
                <el-dialog :title="dialogTitle[operator]" :visible.sync="dialogIsVisible" width="70%">
                    <UserRoleEdit v-model="dialogIsVisible"
                              :operator="operator"
                              :data="currentRow"
                              @loadData="loadData"></UserRoleEdit>
                </el-dialog>
            </el-row>
        </div>
    </div>
...
<script>
    ...
    export default {
        name: "UserRoleList",
        components: {UserRoleQuery, UserRoleEdit},
        created() {
        },
        data() {
            return {
        ...
        methods: {
            ...
            loadData(shadow = {}) {
                ...
                this.$service.get('/system/userRoles', {params: this.params})
                        .then(r => {
            ...
            handleDelete(id) {
                this.$Confirm(this.$t('confirm.delete'), this.$t('confirm.title'), {
                    ...
                    this.$service.delete('/system/userRoles/'+id)
                        .then(r => {
                    if (r.data.code === this.$successCode) {
                    this.$emit('loadData')
                    this.$Message({
