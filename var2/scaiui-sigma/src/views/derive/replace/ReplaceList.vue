  ...
  <div>
    ...
    <el-row type="flex" justify="left">
      <el-form label-position="right" :inline="true" style="width: 100%;">
        <sFormItem3 :label="$t('replace.replaceCode')">
          <el-input v-model="replaceCode" :placeholder="$t('placeholder.default',[$t('replace.replaceCode')])" clearable></el-input>
        </sFormItem3>
        <sFormItem3 :label="$t('replace.replaceType')">
          <sSelect v-model="replaceType" option="CHANGE_TYPE" :placeholder="$t('placeholder.default',[$t('replace.replaceType')])" clearable></sSelect>
        </sFormItem3>
        <sFormItem3 :label="$t('replace.replaceExplain')">
          <el-input v-model="replaceExplain" :placeholder="$t('placeholder.default',[$t('replace.replaceExplain')])" clearable></el-input>
        </sFormItem3>
    ...
    <el-row>
      <el-table style="width: 100%" :data="data.rows" @selection-change="handleSelectionChange" tooltip-effect="light">
        ...
        <el-table-column prop="dataCode" :label="$t('replace.replaceCode')" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column :label="$t('replace.replaceType')" show-overflow-tooltip>
          <template slot-scope="scope">
            <sText option="CHANGE_TYPE" v-model="scope.row.changeType"></sText>
          </template>
        </el-table-column>
        <el-table-column prop="changeDesc" :label="$t('replace.replaceExplain')" show-overflow-tooltip width="200"></el-table-column>
    ...
    <el-row>
      <el-dialog :title="dialogTitle[operator]" :visible.sync="dialogIsVisible" width="80%" :close-on-click-modal="false" @close="closeModal">
        <ReplaceView ref="replaceViewRef" :currentRow="currentRow" :operator="operator" @loadData="loadData" @closeModal="closeModal"></ReplaceView>
      </el-dialog>
    </el-row>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        operator: 'view',
        dialogTitle: {
          "add": this.$t('replace.$title', [this.$t('add')]),
          "edit": this.$t('replace.$title', [this.$t('edit')]),
          "view": this.$t('replace.$title', [this.$t('view')])
        }
      }
    },
    mounted() {
    ...
    methods: {
      // 关闭模态框
      closeModal() {
        this.$refs.replaceViewRef.handleClose();
      ...
      loadData(index) {
        ...
        this.params = Object.assign({
          limit: this.limit,
          offset: this.offset,
          dataCode: this.replaceCode,
          changeType: this.replaceType,
          changeDesc: this.replaceExplain
        });
        this.$service.get('/system/datachangemapping', {params: this.params}).then(r => {
      ...
      handleDeleteMutil() {
        this.$confirm(this.$t('confirm.delete'), this.$t('confirm.title'), {
          ...
          this.$service.delete('/system/datachangemapping/', {data: this.selectedIds})
            .then(r => {
              if (r.data.code === '000000') {
                this.loadData();
                this.$message({
      ...
      handleDelete(id) {
        this.$confirm(this.$t('confirm.delete'), this.$t('confirm.title'), {
          ...
          this.$service.delete('/system/datachangemapping/' + id)
            .then(r => {
              if (r.data.code === '000000') {
                this.loadData();
                this.$message({
